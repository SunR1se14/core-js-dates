...
function getNextFriday(date) {
  ...
  if (daysUntilFri === 0) {
    nextFriday = new Date(date.getTime() + 7 * 24 * 60 * 60 * 1000);
  } else {
    nextFriday = new Date(date.getTime() + daysUntilFri * 24 * 60 * 60 * 1000);
  }
...
function getCountWeekendsInMonth(month, year) {
  ...
  let totalCountWeekends = 0;

  for (let day = 1; day <= totalDays; day += 1) {
    const foundDate = new Date(year, month - 1, day);
    const dayOfWeek = foundDate.getDay();

    if (dayOfWeek === 0 || dayOfWeek === 6) {
      totalCountWeekends += 1;
    }
  }

  return totalCountWeekends;
}
